...
runs.** The matrix's own code, report, and canonical results live here
rather than in the project's shared `results/`/`docs/` pool, specifically
so a future joint-block run doesn't collide with or get confused for one
of these closed runs. §3a is a deliberate, acknowledged exception to that.
