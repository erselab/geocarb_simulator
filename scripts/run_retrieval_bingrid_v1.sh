...
# scoring/REPORT.md step (submit_retrieval_bingrid_v1_postprocess.sbatch,
# which just re-invokes gd_joint_block_matrix.py -- no separate merge
# needed, each array task already writes one complete .pkl) with a SLURM
...
echo "submitted scoring/report job ${POST_JOBID} (runs after ${ARRAY_JOBID} completes: "
echo "gd_joint_block_matrix.py --tag retrieval_bingrid_v1, scoring only, no re-run)"
echo
