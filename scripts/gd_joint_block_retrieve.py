...
def main() -> int:
    ...
    ap.add_argument("--solver", type=str, default=None,
                    ...
                    help="which gert RTSolver both truth generation and the retrieval's own forward model use "
                         ...
                         "K_ssa_lay==0 makes that same composition degenerate. "
                         "'two_stream' (2026-09-15) is a purpose-built solver, not "
                         "'eig_add' with its own n_streams turned down -- measured ~32x "
                         "cheaper than eig_add(n_streams=2) at this band's own hi-res "
                         "wavenumber count (~3.3s vs ~105s per anchor), and the config "
                         "this project actually uses. Still materially more expensive "
                         "than single_scatter (~20-80x per anchor) -- opt in "
