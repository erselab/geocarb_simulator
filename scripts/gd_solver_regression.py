...
Run:  PYTHONPATH=. python3 scripts/gd_solver_regression.py
      PYTHONPATH=. python3 scripts/gd_solver_regression.py --analytic-only
      PYTHONPATH=. python3 scripts/gd_solver_regression.py --configs co2-58-g1,co2p-29-g3
Output: results/gd_solver_regression.pkl  +  docs/SOLVER_REGRESSION.md
"""
...
def out_path(cid, jacobian):
    return REPO_ROOT / "results" / "solver_regression" / f"{cid}_{jacobian}.pkl"
...
def main() -> int:
    ...
    (REPO_ROOT / "results" / "solver_regression").mkdir(parents=True, exist_ok=True)
    ...
    with open(REPO_ROOT / "results" / "gd_solver_regression.pkl", "wb") as f:
        pickle.dump({"scored": scored, "timings": timings, "tol": args.tol}, f)
    doc = REPO_ROOT / "docs" / "SOLVER_REGRESSION.md"
