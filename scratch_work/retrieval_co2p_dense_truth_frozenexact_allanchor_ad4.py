#!/usr/bin/env python3
"""Experiment B, all-frozen-rows-on-anchor-grid variant (2026-09-05,
user: "yes let's do that" -- confirming the extension proposed in
docs/PROJECT_STATUS.md Sec.3). Identical to
retrieval_co2p_dense_truth_frozenexact_anchoralbedo_ad4.py (albedo
frozen exact on its own ANCHOR grid) except ch4_ppb/co_ppb/
h2o_surface_vmr are ALSO now frozen on the anchor grid instead of the
coarser bin_centers grid (--frozen-atmosphere-positions anchor, new
flag).

Sec.3 ruled out albedo's own sub-anchor texture as the driver of the
residual gap at rows 1013-1023 (ad16 didn't help at all). This tests
the other leading candidate directly: are ch4/co/h2o -- frozen on
bin_centers this entire investigation, never tested at anchor
resolution -- the real driver instead?
"""
import os
import pickle
import sys
...
print("dense_frozen_exact atmosphere prior rows (ALL frozen rows on ANCHOR grid this time):", flush=True)
for name in als.STATE_FIELDS:
    src = "structural (free, bin_centers, unchanged)" if name in FREE_ATM_ROWS else "EXACT TRUTH (frozen, ANCHOR grid)"
    print(f"  {name}: {src}", flush=True)
print("  albedo: EXACT TRUTH (frozen, ANCHOR grid)", flush=True)
...
sys.argv = [
    ...
    "--out", str(RESDIR / "co2p-58-g1-ad4-pf-densefrozenexact-allanchor-DENSETRUTH_analytic.pkl"),
]
