#!/usr/bin/env python3
"""Job-array version of retrieval_vs_mode2_truth_structural.py -- windows
distributed via --task-id/--n-tasks (SLURM_ARRAY_TASK_ID/SLURM_ARRAY_TASK_COUNT)
as separate top-level processes, each ALSO using real anchor-level
parallelism (--anchor-workers) inside its own windows -- true two-level
parallelism, validated this session (linearize() 5.28x on a 513-anchor
window; build_forward_state 3.2x on the same).
"""
import os
import pickle
import sys
...
sys.argv = [
    ...
    "--prior-fields", "structural", "--n-windows", "58", "--anchor-density", "16",
    ...
    "--out", str(RESDIR / "co2p_albedo-58-g1-ad16-pf-exact-MODE2TRUTH_structural_analytic.pkl"),
]
