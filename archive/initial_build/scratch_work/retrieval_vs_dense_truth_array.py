#!/usr/bin/env python3
"""Job-array test: prior = true state EXACTLY at each bin center
(--prior-fields exact, native STATE_FIELDS/SURFACE_FIELDS -- NOT
resolution_matched_fields, since --resolution-matched-g-ratio-bins is
deliberately NOT passed here), but the TRUTH IMAGE is Mode 1's dense
truth (500m uniform anchor spacing, full continuous truth) -- so real,
honest sub-bin structure exists that the retrieval's own bin grid cannot
represent, unlike every Mode-2 test this session. This isolates the
actual representability gap using the now-correct (footprint-integrated,
no spectral interpolation) forward model throughout -- 2026-09-02, user.

Windows distributed via --task-id/--n-tasks (SLURM_ARRAY_TASK_ID/
SLURM_ARRAY_TASK_COUNT) as separate top-level processes, each using real
anchor-level parallelism (--anchor-workers) inside its own windows.
"""
import os
import pickle
import sys
...
with open(REPO_ROOT / "scratch_work" / "whole_slit_truth_v1.pkl", "rb") as f:
    truth = pickle.load(f)
A_TRUTH = truth["dense"]   # Mode 1 -- real sub-bin structure, unlike "representative"
...
def _fake_band_setup_cached(fpa, atm_center, absco, geo, solar, snr, n_lookup_samples,
    ...
    print(f"  [FAKE band_setup_cached] injecting precomputed Mode-1 dense "
...
# Deliberately NO --resolution-matched-g-ratio-bins / --resolution-matched-
# anchor-density here: resolution_matched_active stays False, so
# --prior-fields exact resolves to als.PRIOR_FIELD_SETS["exact"] ==
# als.STATE_FIELDS (the raw native truth), evaluated exactly at each bin's
# own position (prior_anchor_density=None, state_spec_from_scene's
# default) -- "prior = true state at the bin centers", matching the truth
# image's own field functions exactly at those points, but NOT in between
# (where the dense truth has real, unrepresented structure).
sys.argv = [
    ...
    "--out", str(RESDIR / "co2p_albedo-58-g1-ad16-pf-exact-DENSETRUTH_analytic.pkl"),
]
