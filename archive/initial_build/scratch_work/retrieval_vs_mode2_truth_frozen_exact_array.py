#!/usr/bin/env python3
"""Ablation #3 (2026-09-04, user): isolate frozen-row contamination from
the structural-prior experiment
(`retrieval_vs_mode2_truth_structural_array.py`).

Same config in every other respect (free=co2_ppm,p_surface_hpa,albedo;
g_ratio=1; anchor_density=16; Mode-2 representative truth injected via the
same fake `_band_setup_cached`) -- the ONLY thing that changes is the
ATMOSPHERE PRIOR for the three FROZEN rows (ch4_ppb, co_ppb,
h2o_surface_vmr): here they are fixed at the exact truth
(`als.STATE_FIELDS`) instead of the structural prior
(`als.STATE_FIELDS_PRIOR`). The two FREE atmosphere rows (co2_ppm,
p_surface_hpa) keep their structural (imperfect) prior unchanged, and
albedo's own prior stays structural too (`SURFACE_FIELDS_PRIOR`,
unaffected) -- so this isolates exactly one variable: does the imperfect
prior on the rows we are NOT retrieving explain the large bin-center
error, independent of any genuine free-parameter degeneracy?

If bin-center error collapses relative to the plain structural run,
frozen-row contamination (not a free-parameter degeneracy) is the
dominant driver. If it doesn't change much, the error is coming from
somewhere else (the free rows' own imperfect prior interacting with
degeneracy/regularization, non-convergence, etc. -- see the other
diagnostics on the list).

No library code touched: `als.PRIOR_FIELD_SETS`/`SURFACE_PRIOR_FIELD_SETS`
are extended with one new throwaway entry ("frozen_exact") at this
driver's own call site, exactly the registry's own documented extension
point ("adding a new imperfect prior means adding one entry here").
"""
import os
import pickle
import sys
...
# -- Build the "frozen rows exact, free rows structural" atmosphere prior --
FREE_ATM_ROWS = {"co2_ppm", "p_surface_hpa"}   # albedo isn't in STATE_FIELDS
_merged_fields = {
...
# albedo is free here too -- keep its own prior exactly as it was in the
# plain structural run (SURFACE_FIELDS_PRIOR), not exact -- this ablation
# only touches the FROZEN atmosphere rows.
als.SURFACE_PRIOR_FIELD_SETS["frozen_exact"] = als.SURFACE_FIELDS_PRIOR

print("frozen_exact atmosphere prior rows:", flush=True)
for name in als.STATE_FIELDS:
...
sys.argv = [
    ...
    "--prior-fields", "frozen_exact", "--n-windows", "58", "--anchor-density", "16",
    ...
    "--out", str(RESDIR / "co2p_albedo-58-g1-ad16-pf-frozenexact-MODE2TRUTH_structural_analytic.pkl"),
]
