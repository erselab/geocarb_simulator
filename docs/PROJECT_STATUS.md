...
## 20. Truth is now generated by calling the retrieval's OWN forward model, not a separately-discretized pipeline (2026-09-13)

Sec.15/19's chain of representability investigations kept finding a
residual that shrank as things got matched better, but never quite
reached zero. Root cause, finally isolated with a `--resolution-
matched-*` diagnostic run to a full apples-to-apples comparison
(single window, `co2_ppm` only, everything else frozen at truth,
band-limited to the retrieval's own state-bin grid via
`--resolution-matched-g-ratio-bins`): `render_at_anchors` genuinely
IS "the one shared forward model" its own docstring claims -- both
truth generation (`_band_setup`) and the retrieval's own
`build_forward_state` call it -- but each caller picks its OWN anchor
grid independently. Truth's anchor grid is a fixed `dx_km=0.5`
(`_band_setup`'s own hardcoded default -- `--n-lookup-samples` turned
out to be VESTIGIAL for this, a stale parameter left over from the
retired `build_lookup_radiance` mechanism, never actually wired to
`dx_km`) or `n_lookup_samples=20000`-derived under
`--resolution-matched-*`; the retrieval's own grid comes from
`--anchor-density`. Even with field VALUES matched exactly (Sec.19's
fix) and both grids fine, `render_at_anchors`'s RT-per-anchor +
footprint-average quadrature is sensitive to the SPECIFIC anchor
positions, not just how fine they are -- two different anchor grids
of the same continuous field don't quite agree. Measured directly:
residual dropped from rms=0.0079 (grid mismatch) to 0.0024 (bin-grid
matched, ad=4) to 0.0015 (bin-grid matched, ad=16) -- shrinking, never
zero, exactly the signature of two similar-but-not-identical
quadrature grids.

User: "I would like to modify the truth generation pipeline so that
we first specify a truth grid and then generate the observation image
through the retrieval pipeline. That will allow us to match exactly"
-- "The retrieval forward model" -- "Please also make that method of
generation of truth the default." `_solve_window` now builds `y_true`
by constructing a fully-frozen (`free=()`) `StateSpec` at
`als.STATE_FIELDS`/`SURFACE_FIELDS` (true field functions, unaffected
by whatever `--prior-fields` the retrieval itself uses) on THIS
window's own `anchor_etas`, then calling `build_forward_state` --
literally the same function, same grid, same `state_interp` kind the
retrieval's own hires solve uses -- instead of slicing the
independently-rendered `band["A"]`. `n_free=0` needed no code fix:
verified directly (`StateSpec.interp_to` with `x=np.array([])`) that a
fully-frozen spec already works -- `unpack()`/`interp_to()` never
touch `x` for a frozen row, so the empty-array case was already
correct, just never exercised.

Validated on the real pipeline (task 1, `co2_ppm` free, everything
else frozen, `--prior-fields structural`): `resid_hires_rms` dropped
to **1.37e-07**, `chi2_hires_reduced` to **7.8e-11** -- genuine
machine-precision agreement, not just "small," confirming the new
default actually closes the gap the whole resolution-matched
diagnostic chain could only ever narrow.

`--legacy-truth` reverts to the old `band["A"]`-slice behavior, for
reproducing or comparing against any pre-2026-09-13 result. `band["A"]`
...
## 36. Noise, the solver policy, the truth cache, and the single- vs multi-band comparison study (2026-09-22 .. 09-25)

* **Solver policy.** `--solver` defaults to `xrtm` when aerosol is free/requested and `single_scatter` otherwise: `SingleScatterSolver`'s only added-scattering source is aerosol, so without it the radiance is exactly absorption/extinction only (Rayleigh enters through tau_total), and XRTM two-stream costs ~32x per anchor. The low-level `simulate_spectrum`/`spectrum_and_jacobian` defaults were flipped to `xrtm` on 09-23 (commit adc6f3a) and callers that relied on them silently rendered truth with XRTM (OOM at 60 GB); `gd_per_row_retrieve.render_solver()` now pins the truth render.
* **Truth cache v8** (`geocarb_gert/truth_cache.py`): entries carry metadata (inputs, git head, Slurm ids, host, argv) and are served only on an exact metadata match; `scripts/list_truth_cache.py` lists them.
* **Geometry config** (`--geometry-config`, `scripts/gd_export_geometry_config.py`): explicit per-tile eta bounds and `bin_centers` (optional `rows_by_fpa`, optional per-tile `overlap`) so single- and multi-band runs share tiles and bins bit for bit (verified for all 33 FPA2 tiles). Tilings: FPA0+FPA2 33 tiles, FPA0+FPA1 60, FPA0+FPA3 47 (tile width follows the largest keystone of the bands in the pair; the 4-band tiling is 33 tiles and matches FPA0+FPA2 in 32 of 33).
* **Noise.** `--noise-seed N` (both drivers): signal-dependent shot noise, variance N0^2 + N1|I| from each band's `geocarb_noise_model`, drawn on the noiseless representative truth; the draw is keyed on (seed, fpa, row range) so a tile's single- and multi-band runs see the same standard-normal draw. Seeds 1-3 were run for all pairs (`NOISE_SEED` in the sweep sbatch scripts).
* **Results** (realistic prior, no aerosol, pooled boundary-trimmed rms, single-band / with FPA0; noisy = mean of 3 seeds): FPA2 CO2 0.161/0.136 ppm (noisy 1.70/1.45), p 0.141/0.037 hPa (1.29/0.47), H2O 6.3e-6/5.1e-6, T 0.0069/0.0062 K; FPA1 CO2 0.201/0.104 (2.33/1.90), p 0.344/0.046 (2.55/0.48); FPA3 CH4 0.88/0.80 ppb (8.7/6.9), CO 0.181/0.159 (2.07/2.05), p 0.233/0.047 (2.39/0.48). Single-band pressure with noise is worse than the prior's own error (0.98 hPa); with FPA0 it is ~0.47. Gases with no lines in the fitted bands (CH4/CO in FPA0+FPA1, CO2 in FPA3) come back exactly equal to the prior (zero Jacobian).
* **Figures/scripts.** `plot_singleband_vs_multiband.py` (`--pair`, `--noise-seed`, `--sb-driver`), `plot_error_histograms.py`, `plot_error_pairplots.py`, `plot_noise_vs_signal.py`, `plot_state_with_fpa_images.py`, `render_fpa_images.py` + `plot_fpa_images.py`, `plot_keystone_maps.py`, `plot_bins_anchors.py`. FPA1 and FPA3 have reversed dispersion (final beamsplitters); their columns are flipped in cross-FPA plots.
* **Shareable summary:** `docs/geocarb_joint_retrievals_summary.html` (standalone, figures embedded).


## 37. Single band through the multi-band code: most of the albedo "puzzle" was the anchor grid (2026-09-25)

`gd_multiband_window.py --fpas 2` runs one band through the multi-band code (`--geometry-config`, `GEOM` in the sweep sbatch). The single-band driver and the multi-band code use different albedo anchor grids ("cover" anchors span every pixel of every padded row, keystone included). Noiseless albedo rms, separate single-band driver / same-code single band / with FPA0: FPA2 0.0074 / 0.0111 / 0.0136, FPA1 0.0140 / 0.0190 / 0.0199, FPA3 0.0094 / 0.0113 / 0.0142 (`scripts/compare_albedo_codepaths.py`). The gap shrinks from 1.4-1.8x to 1.05-1.3x; the other state variables barely move. A 1.2-1.3x single-band advantage remains for FPA2/FPA3 (unexplained); the anchor-grid cause was not isolated. All single-vs-multi figures now use the same-code single band (`--sb-driver` restores the old arm).


## 38. Aerosol types with per-band Mie properties; `--aerosol` requires `--aerosol-type` (2026-09-25)

* `geocarb_gert/aerosol_mie.py`: Bohren & Huffman Mie over an assumed lognormal size distribution and refractive index gives (SSA, g, extinction relative to O2-A) at the four band centres for smoke, sulfate, sea_salt, dust, cloud_water (values entered from memory -- replace with an authoritative table). Fine-mode smoke/sulfate have SWIR AOD of only 12/6/4% (smoke) of O2-A at 1.6/2.06/2.3 um; the coarse types are nearly grey.
* `--aerosol-type` names now mean these properties, with **scene AOD and `amplitude_aerosol` defined at O2-A** (as the scene docstrings always said). The legacy scheme (O2-A slot for FPA0, ONE 1.6 um slot shared by FPA1-3, AOD at 1.6 um) is `registry_<type>`; **every aerosol result before 09-25 (the FPA0+FPA2 aerosol arm) used it**. `gd_multiband_window.py --aerosol` errors without `--aerosol-type`; `--check-aerosol` prints the per-band table. The single-band driver has no such flag.
* **4-band experiments:** FPA0-3 jointly, all species free. No-aerosol arm (noiseless + seeds 1-3, 33 tiles) done; tile 12: 698 unknowns, 16 min, ~31 GB. Aerosol probe (smoke, tile 12, 744 unknowns): 4.3 h on 16 CPUs, ~200 GB peak -> the four aerosol sweeps are several days at the FD pressure Jacobian (Sec.40).


## 39. A tapering-edge cloud in the truth; O2-A-only non-scattering retrieval (2026-09-25/26)

`geocarb_gert/cloud_scene.py`: a liquid cloud (`cloud_water`) as a Gaussian pressure layer (20 hPa) whose optical depth has a 30 km plateau and cosine-tapered edges fading over 150 km, centred at +700 km; 3 altitudes (850/600/350 hPa) x 3 optical depths (0.5/2/8). `gd_multiband_window.py --cloud-tau T --cloud-p-hpa P` renders the TRUTH with the cloud (XRTM) while the retrieval stays clear-sky `single_scatter` (tau 0 = the matching clear control). `render_cloud_images.py` renders clear/cloudy detector sub-images in all four bands (1 km anchors, XRTM; ~7/3.4/2.9/2.6 s per anchor per band). Findings: the cloud brightens the bands 1.5x (O2-A) to ~8x (FPA3) for mid/tau 2, absorption lines by 4-15x; cloud contrast is set by the underlying albedo patch (bright desert hides the right-hand edge). FPA0-only fit (p_surface + O2-A albedo, tiles 42-54): low clouds give a smooth pressure response following the taper (-50..-125 hPa); thick mid/high clouds a plateau at the cloud-top pressure; thin/moderate mid/high clouds give chaotic tile-to-tile pressures (local minima) but a residual that rises along the taper and exceeds 1.5x the clear control at cloud optical depth ~0.015-0.06 (high), 0.06-0.09 (mid), 0.4-1.4 (low). Noise (seeds 1-3) changes almost nothing (the model misfit dwarfs the noise). Scripts: `plot_cloud_images.py`, `plot_cloud_o2a.py [--noise]`.


## 40. Analytic surface-pressure Jacobian under XRTM with aerosol (2026-09-26)

Until now `p_surface_dI_dparam` used a 2-call RT finite difference whenever an aerosol row was present (the only FD column in the aerosol solve). The no-aerosol composition (gas pressure/T/H2O paths through `K_mol_lay`, Rayleigh optical depth through `K_ray_lay`) misses two effects once an aerosol layer exists: (e) the Gaussian aerosol layer weights `aer_frac_l = w_l/sum w`, `w_l = exp(-0.5((p_l - height)/sigma)^2)`, move with the layer pressures (`p_l = sigma_l p_sfc`); (f) in a layer holding Rayleigh and aerosol the Rayleigh fraction sets the mixed phase-function moments, `dchi/dtau_ray = (chi_ray - chi)/tau_scat_total`, which XRTM returns only if seeded. **gert** (`psurf-analytic-jacobian`, merged): `combined_layer_coef_ray_deriv` and `XRTMSolver(psurf_seed=True)`, ONE extra derivative slot seeded along ln(p_sfc) in every layer, exposed as `RTResult.K_coef_ray_lnp` / `ForwardResult.K_coef_ray_lnp_hires`. **geocarb_gert/jacobians.py**: `_p_surface_dI_dparam_xrtm_aerosol` = no-aerosol composition + `sum_l K_aer_lay[l] tau_aer_col dfrac_l/dlnp/p_sfc` + `K_coef_ray_lnp/p_sfc`; dispatched when `solver == "xrtm"` and the slot exists; `GEOCARB_PSURF_FD=1` restores the FD. **Validation** (`scripts/check_psurf_jacobian.py`, 4 bands x anchors at 750/948/1001/1003 hPa, smoke Mie): relative L2 error vs the FD 5e-6 .. 1.2e-3, cosine 1.000000; without the two new terms the composition is off by up to 30% (FPA0 at the haze peak, cosine 0.9957); per-anchor Jacobian evaluation ~2.4x faster (10 s vs 24 s FPA0, 4 s vs 9 s FPA3). Not modelled: the layer-altitude dependence of XRTM's pseudo-spherical correction. **End to end** (tile 12, 4 bands, smoke, 744 unknowns, job 1977659 vs the earlier FD probe): solve 15515 s -> 10573 s (**1.47x**), rms_resid 4.519e-3 vs 4.518e-3; retrieved state differs by rms 0.0066 ppm CO2, 0.0097 hPa p_surface, 3.4e-7 H2O (relative 5e-5), 30 Pa in height_aerosol (of ~6e4), 1e-8 in amplitude (0.3%) -- far below the retrieval errors. The per-anchor Jacobian evaluation is 2.4x faster but a Gauss-Newton iteration is only ~1.3-1.5x, because RT is ~60% of an iteration (the rest is the detector operator over 744 columns, the forward trial steps, interprocess traffic). Profile: see below.
